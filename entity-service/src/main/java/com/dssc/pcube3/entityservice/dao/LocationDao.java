...
import com.dssc.pcube3.entityservice.entity.Location;
...
public interface LocationDao extends Neo4jRepository<Location, Long> {
    @Query("MATCH (n:LOC)-[r:OpenRel]-(m) where (n.eid={eid}) return n,m.name as nodeName,m.summary as des")
    List<NodeDTO> getNodesList(String eid);

    @Query("MATCH (n:LOC) where (n.eid={eid}) return n,n.name as nodeName,n.summary as des LIMIT 1")
    NodeDTO getTargetNode(String eid);

    @Query("MATCH (n:LOC)-[r:OpenRel]-(m) where (n.eid={eid}) return n," +
            "n.name as source,r.name as value,m.name as target")
    List<LinkDTO> getLinksList(String eid);

    @Query("MATCH (n:LOC)-[r:OpenRel]-(m) RETURN COUNT(*)  AS COUNT")
    Integer getLinksCount();

    @Query("MATCH (n:LOC) RETURN count(*) as COUNT")
